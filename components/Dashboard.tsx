...
const Dashboard: React.FC<DashboardProps> = ({ leads, profile, onNavigateToLead, onViewLeads }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-10">
      {/* TRIAL BANNER */}
      {!profile?.is_active && (
        <div className={`mb-8 p-5 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 border-2 shadow-sm ${trialExpired ? 'bg-red-50 border-red-200 text-red-900' : 'bg-blue-50 border-blue-200 text-blue-900'}`}>
          <div className="flex items-center gap-4">
            <div className={`w-10 h-10 shrink-0 rounded-full flex items-center justify-center font-black text-sm ${trialExpired ? 'bg-red-200' : 'bg-blue-200'}`}>!</div>
            <p className="text-sm font-bold uppercase tracking-widest text-center md:text-left leading-relaxed">
              {trialExpired ? 'Free Trial Ended' : `${trialDaysLeft} Days remaining in trial`}
            </p>
          </div>
          {trialExpired && <span className="text-xs font-black underline cursor-pointer hover:text-red-700">Upgrade Now</span>}
        </div>
      )}
